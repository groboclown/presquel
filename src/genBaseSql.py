    ...
    def load(self):
        ...
        if self.branch is not None:
            self.problems.extend([str(branch)
                                  for branch in self.branch.branch.problems])
...
if __name__ == '__main__':
    ...
    # FIXME use a command line parser to allow for:
    #  select 1 or more packages, possibly its version (a/b/abc@1.2.3 ?)
    #  select the output directory.
    #  select the db platform
